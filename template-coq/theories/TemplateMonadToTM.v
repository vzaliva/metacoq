...
Definition printTerm (name  : ident): TemplateMonad unit :=
  (tmBind (tmQuote name) tmPrint).

Set Boolean Equality Schemes.
Print Ast.term.
Definition sort := universe.
...
Axiom CoqOpid_beq : forall (a b : CoqOpid), bool.
...
Module STermVarInstances.
  Let fvN3 : FreshVars (N*name) _ := 
    @varImplDummyPair.freshVarsNVar  _ _ _ (freshVarsN 3) nAnon.
...
(* assuming [p]'s type is of the form,
[forall (t1:T1) (t2: T2) ....(tn: Tn), TemplateMonad unit] 
and [p] is of the form [λ (t1:T1) (t2: T2) ....(tn: Tn), ...]
assuming that ti are all multiples of 3. This is typically done
in the translation from Coq term to SquiggleEq representation
 *)
Import BasicAst.
...
Notation "∖tmQuote" :=10%nat.
Notation "∖tmReturn" :=0%nat.
...
Locate TM.
Print TM.
Definition termQuote : STerm  := (mkConst "MetaCoq.Template.Ast.term").
Require Import SquiggleEq.terms2.
Require Import SquiggleEq.terms.

Fixpoint transC (c: list Arg) (p: STerm) : STerm :=
  match p with
  | vterm v => vterm v
  | oterm (CConstruct {| inductive_mind := "MetaCoq.Template.TemplateMonad.Core.TemplateMonad"; inductive_ind :=0 |} ∖tmQuote)
          [bterm [] _ (*type *); bterm [] t] =>
    oterm (CConstruct {| inductive_mind := "MetaCoq.Template.TemplateMonad.Extractable.tmReturn"; inductive_ind :=0 |} ∖tmReturn)
          [bterm [] termQuote;
             bterm [] (tvmap vprime t)]
(*  | mkLam x A b => p not yet implemented *)
  | mkApp f a =>
    mkApp (trancC c f)
  | oterm o lbt =>
    oterm o (map (btMapNt (transC c)) lbt)
  end.


Print TemplateMonad.
Definition quoteArg (a:Arg) : list Arg :=
  let '(v, (type,sort)) :=a in
  [a; (vprime v, (termQuote, None))].

Definition  trans (p: STerm) : STerm :=
  let (body, args) := getHeadLams p in
  mkLamLS (flat_map quoteArg args) (transC args body).
